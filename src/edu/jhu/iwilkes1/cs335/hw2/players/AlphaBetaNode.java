...
public class AlphaBetaNode {
	...
	private Board board;
	private HashMap<Move, AlphaBetaNode> successors;
	...
	public AlphaBetaNode(Board board, AlphaBetaNode parent, int remainingDepth, boolean isPlayerBlack) {
		...
		this.isMaxNode = (this.isPlayerBlack && this.board.getTurn() % 2 == 1)
				|| (!this.isPlayerBlack && this.board.getTurn() % 2 == 0);

		if (this.parent == null) {
			this.successors = new HashMap<Move, AlphaBetaNode>();
		} else {
		...
		if (endState != Chip.NONE) {
			...
			for (Move m: this.board.getLegalMoves()) {
				...
				if (this.successors != null) {
					this.successors.put(m, nextNode);
				}
			}
	 ...
	 */
	public HashMap<Move, AlphaBetaNode> getSuccessors() {
